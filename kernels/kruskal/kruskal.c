...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        klee_assume(edges[i].u >= 0); klee_assume(edges[i].u < n);
        klee_assume(edges[i].v >= 0); klee_assume(edges[i].v < n);
        klee_assume(edges[i].u != edges[i].v); klee_assume(edges[i].wt >= 0);
    }
